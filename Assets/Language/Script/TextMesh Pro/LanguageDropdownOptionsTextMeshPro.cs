...
public class LanguageDropdownOptionsTextMeshPro : MonoBehaviour
{
    [Header("Settings")]
    [SerializeField] private TMP_Dropdown targetDropdown; // Required Dropdown component.
    [SerializeField] private TMP_Text childTexts; // Dropdown's base text component.
    [SerializeField] private TMP_Text displayText; // Dropdown Display Text Component.
    public List<LanguageSelectedTextMeshPro> languageSelected = new List<LanguageSelectedTextMeshPro>(); // List of language options.
    ...
    private void ReadFile(string filePath)
    {
        ...
        foreach (string line in lines)
        {
            ...
            if (line.StartsWith("id:"))
            {
                ...
                foreach (LanguageSelectedTextMeshPro p in languageSelected)
                {
                    if (p.value == id)
                    {
                        if (Reverse)
        ...
        foreach (LanguageSelectedTextMeshPro languageOption in languageSelected)
        {
            TMP_Dropdown.OptionData optionData = new TMP_Dropdown.OptionData(); // Create an OptionData object for the new option.
            ...
            if (idIndex >= 0)
            {
                ...
                if (id == valueID)
                {
                    ...
                    if (FontIndex >= 0)
                    {
                        ...
                        {
                            childTexts.font = fontListObject.FontListTextMeshPro[Font - 1]; // Set the target text's font to the font at the specified index in the font list.
                            displayText.font = fontListObject.FontListTextMeshPro[Font - 1]; // Set the target text's font to the font at the specified index in the font list.
                        }
                    }
                    ...
                    if (AliIndex >= 0)
                    {
                        ...
                        if (Alignment == 1)
                        {
                            childTexts.alignment = TextAlignmentOptions.Left;
                            displayText.alignment = TextAlignmentOptions.Left;
                        }
                        else if (Alignment == 2)
                        {
                            childTexts.alignment = TextAlignmentOptions.Center;
                            displayText.alignment = TextAlignmentOptions.Center;
                        }
                        else if (Alignment == 3)
                        {
                            childTexts.alignment = TextAlignmentOptions.Right;
...
[System.Serializable]
public class LanguageSelectedTextMeshPro
{
