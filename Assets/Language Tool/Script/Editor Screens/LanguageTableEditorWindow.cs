...
namespace LanguageTools.Editor
{
    public class LanguageTableEditorWindow : EditorWindow
    {
        ...
        /// </summary>
        [MenuItem("Tools/Language Tool/Language Table Editor Window")]
        ...
        private void RenderVisibilityToggles()
        {
            ...
            if (GUILayout.Button(new GUIContent("A", "Select all rows."), GUILayout.Width(20)))
            {
                GUI.FocusControl(null);
            ...
            if (GUILayout.Button(new GUIContent("A", "Select all columns."), GUILayout.Width(20)))
            {
                GUI.FocusControl(null);
            ...
            if (GUILayout.Button(new GUIContent("T", "Select all 'True' columns."), GUILayout.Width(20)))
            {
                GUI.FocusControl(null);
                Undo.RecordObject(this, "Select Active Columns");
                selectedColumns.Clear();

                // Safety check to ensure table is valid.
                if (tableData != null && tableData.Length > 3)
                {
                    int columnCount = tableData[0].horizontalTable.Length;

                    // Iterate through all columns.
                    for (int j = 0; j < columnCount; j++)
                    {
                        // Always include columns 0 and 1.
                        if (j == 0 || j == 1)
                        {
                        ...
                        // Get value from row 3 (boolean row).
                        string value = tableData[3].horizontalTable[j];

                        // Check if column is active (True).
                        if (string.Equals(value, "True", StringComparison.OrdinalIgnoreCase))
                        {
                            selectedColumns.Add(j); // Add column index to selection.
                        }
                    }
                }
        ...
        private void RenderColumnHeaders(Rect visible, Event e)
        {
            ...
            if (showOnlySelected)
            {
                ...
                for (int visualIndex = 0; visualIndex < visibleColumns.Count; visualIndex++)
                {
                    int j = visibleColumns[visualIndex];
                    Rect cellRect = new(xOffset, 0, columnWidths[j], MIN_ROW_HEIGHT);

                    if (cellRect.xMax < visible.x || cellRect.x > visible.xMax)
                    {
        ...
        private void RenderRowCells(int rowIndex, float yOffset, Rect visible, Event e)
        {
            ...
            if (showOnlySelected)
            {
                ...
                for (int visualIndex = 0; visualIndex < visibleColumns.Count; visualIndex++)
                {
                    ...
                    if (!IsRectVisible(cellRect, visible))
                    {
                        xOffset += columnWidths[j];
                        continue;
                    }
